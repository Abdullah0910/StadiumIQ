    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl flex flex-col h-[520px] shadow-2xl overflow-hidden">
      ...
      <div className="bg-indigo-950/60 p-4 border-b border-slate-800 flex items-center justify-between">
        ...
        <button
          ...
          className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
          title="Clear Chat"
      ...
      {/* Suggestion Chips */}
      <div className="px-4 py-2 bg-slate-900 border-t border-slate-800/50 flex gap-2 overflow-x-auto no-scrollbar whitespace-nowrap" role="group" aria-label="Suggested inquiries">
        {getSuggestionChips(role).map((chip, idx) => (
          <button
            key={idx}
            onClick={() => handleSendMessage(chip)}
            className="px-3 py-1 bg-slate-800 hover:bg-slate-750 text-slate-300 border border-slate-750 text-[11px] rounded-full transition-colors shrink-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            {chip}
          </button>
        ))}
      ...
      >
        <input
          ...
          aria-label={`Ask Gemini co-pilot as ${role}`}
          className="flex-1 bg-slate-900 border border-slate-800 text-xs text-slate-100 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
        />
        <button
          type="submit"
          disabled={!inputText.trim() || isLoading}
          aria-label="Send inquiry to Gemini"
          className="p-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 text-white disabled:text-slate-500 rounded-xl transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
